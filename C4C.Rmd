...
sum_data <- read_csv(here("DATA", "Sum_Crops.csv"))
...
cotton_SOC <- ggplot ()+
  ...
  geom_text(aes(x = 16, y = 490000, label = "Regenerative"))+
  geom_line(data = cotton_organic_SOC, aes(x = Year, y = End_SOC), col = "darkorange2")+
  geom_text(aes(x = 16, y = 420000, label = "Organic"))+
...
cotton_gwp <- ggplot()+
  ...
  geom_text(aes(x = 16, y = 200, label = 'Regenerative'))+
  geom_line(data = cotton_organic_SOC, aes(x = Year, y = GWP), col = "darkorange2")+
  geom_text(aes(x = 16, y = 82000, label = 'Organic'))+
  labs(title = "Cotton", y = element_blank(), x = element_blank())+
  theme_classic()+
  theme(plot.title = element_text(hjust = 0.5)) # center the graph
...
#######
# t-test for organic vs regenerative 
#######

# KERNZA

kernza_3_soc_t <- t.test(all_data$dSOC[55:72], kernza$dSOC[37:54])
kernza_3_soc_t
#there is no significant difference in kernza crop between organic and regenerative in dSOC (Minnesota)

kernza_3_gwp_t <- t.test(all_data$GWP[55:72], kernza$GWP[37:54])
kernza_3_gwp_t
#there is a significant differences in kernza crop between organic and regenerative in GWP (Minnesota)


#COTTON
cotton_3_soc_t <- t.test(all_data$dSOC[217:234], all_data$dSOC[145:162])
cotton_3_soc_t
#there is a significant difference in cotton crop between organic and regenerative in dSOC (India)

cotton_3_gwp_t <- t.test(all_data$GWP[217:234], all_data$GWP[145:162])
cotton_3_gwp_t
#there is a significant difference in cotton crop between organic and regenerative in GWP (India)


#MANGO 
mango_3_soc_t <- t.test(all_data$N2O_CO2e[639:656], all_data$N2O_CO2e[657:674])
mango_3_soc_t
#there is no significant difference in mango crop between organic and regenerative in dSOC (Nicaragua)

mango_3_gwp_t <- t.test(all_data$CH4_CO2e[639:656], all_data$CH4_CO2e[657:674])
mango_3_gwp_t
#there is a significant difference in mango crop between organic and regenerative in GWP (Nicaragua)


# BISON
bison_3_soc_t <- t.test(all_data$dSOC[567:584], all_data$dSOC[585:602])
bison_3_soc_t
#There is a significant difference in bison grazing between organic and regenerative in dSOC (South Dakota)

bison_3_gwp_t <- t.test(all_data$GWP[567:584], all_data$GWP[585:602])
bison_3_gwp_t
#There is no significant difference in bizon grazing between organic and regenerative in GWP (South Dakota)
...
kernza_uk_min_regen_gwp_t
# under organic practices all of the sites are significantly different GWP values for kernza
...
cotton_nut_SOC_t<- t.test(all_data$dSOC[217:234], all_data$dSOC[343:360])
cotton_nut_SOC_t
cotton_nut_GWP_t<- t.test(all_data$GWP[217:234], all_data$GWP[343:360])
cotton_nut_GWP_t

bison_nut_SOC_t<- t.test(all_data$dSOC[585:602], all_data$dSOC[3:20])
bison_nut_SOC_t
bison_nut_GWP_t<- t.test(all_data$GWP[2585:602], all_data$GWP[3:20])
bison_nut_GWP_t

kernza_nut_SOC_t<- t.test(all_data$dSOC[55:72], kernza_compost$dSOC[3:20])
kernza_nut_SOC_t
kernza_nut_GWP_t<- t.test(all_data$GWP[55:72], kernza_compost$GWP[3:20])
kernza_nut_GWP_t

mango_nut_SOC_t<- t.test(mango_compost$dSOC[1:18], mango_compost$dSOC[37:54])
mango_nut_SOC_t
mango_nut_GWP_t<- t.test(mango_compost$GWP[1:18], mango_compost$GWP[37:54])
mango_nut_GWP_t
...
bison_three_s<-sum(bison_crop$dSOC[63:80])
...
crop_sens_SOC_india<- ggplot(cropping_data_cotton, aes(y = SOC, x = Crop, fill = Crop))+
  geom_bar(stat = "identity", position = "dodge", show.legend = "False")+
  labs(y =element_blank(), title= "Cotton in India",x= element_blank())+
    scale_fill_manual(values = c("deepskyblue4","deepskyblue3","deepskyblue2", "deepskyblue","skyblue"))+
  theme_classic()
...
crop_sens_SOC_sd<- ggplot(cropping_data_bison, aes(y = SOC, x = Crop, fill = Crop))+
  geom_bar(stat = "identity", position = "dodge", show.legend = "False")+
  labs(y =element_blank(), title= "Grazing in South Dakota",x= element_blank())+
  scale_fill_manual(values = c( "darkslategray","darkslategray4","darkseagreen4", "darkseagreen3", "darkseagreen2"))+
  theme_classic()
...
cropping_data_mango$Crop <- factor(cropping_data_mango$Crop,levels = c("Mango", "Two Crops", "Regenerative"))

crop_sens_SOC_nic<- ggplot(cropping_data_mango, aes(y = SOC, x = Crop, fill = Crop))+
  geom_bar(stat = "identity", position = "dodge", show.legend = "False")+
  labs(y =element_blank(), title="Mango in Nicaragua",x= element_blank())+
  scale_fill_manual(values = c("darkorange4", "darkorange3","darkorange"))+
  theme_classic()
...
crop_sens_SOC_mn<- ggplot(cropping_data_kernza, aes(y = SOC, x = Crop, fill = Crop))+
  geom_bar(stat = "identity", position = "dodge", show.legend = "False")+
  labs(y =element_blank(), title="Kernza Wheat in Minnesota",x= element_blank())+
  scale_fill_manual(values = c("aquamarine4","aquamarine3","aquamarine2", "aquamarine"))+
  theme_classic()
...
cropping_gwp<-ggarrange(crop_sens_gwp_india,crop_sens_gwp_sd,crop_sens_gwp_nic,crop_sens_gwp_mn)

annotate_figure(cropping_gwp,
                top = text_grob("Sensitivity Analysis of Crop Rotation: Total Change in Global Warming Potential",face="bold", size = 14),
                left = text_grob("kg CO2e/ha",rot=90)
)




cropping_soc<-ggarrange(crop_sens_SOC_india,crop_sens_SOC_sd,crop_sens_SOC_nic,crop_sens_SOC_mn)


annotate_figure(cropping_soc,
                top = text_grob("Sensitivity Analysis of Crop Rotation: Total Change in SOC",face="bold", size = 14),
                left = text_grob("Total Change in SOC(kg/ha)",rot=90)
)
...
tillage_india_SOC_t<- t.test(all_data$dSOC[217:234], all_data$dSOC[325:342])
tillage_india_SOC_t
tillage_india_GWP_t<- t.test(all_data$GWP[217:234], all_data$GWP[325:342])
tillage_india_GWP_t

tillage_china_SOC_t<- t.test(all_data$dSOC[181:198], all_data$dSOC[477:494])
tillage_china_SOC_t
tillage_china_GWP_t<- t.test(all_data$GWP[181:198], all_data$GWP[477:494])
tillage_china_GWP_t

tillage_texas_SOC_t<- t.test(all_data$dSOC[199:216], all_data$dSOC[513:530])
tillage_texas_SOC_t
tillage_texas_GWP_t<- t.test(all_data$GWP[199:216], all_data$GWP[513:530])
tillage_texas_GWP_t

tillage_peru_SOC_t<- t.test(all_data$dSOC[235:252], all_data$dSOC[495:512])
tillage_peru_SOC_t
tillage_peru_GWP_t<- t.test(all_data$GWP[235:252], all_data$GWP[495:512])
tillage_peru_GWP_t
