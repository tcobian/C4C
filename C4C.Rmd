...
grid.arrange(ch4_n20_kernza_plot, ch4_n20_cotton_plot, ch4_n20_mango_plot, ch4_n20_bison_plot, ncol = 2, nrow = 2)
...
mango_india_regen_gwp<- ((sum(mango$End_SOC[1:18])/12)*44) + (((sum(mango$CH4[1:18])/12)*16)*28) + (((sum(mango$N2O_flux[1:18])/28)*44)*265)

mango_india_organic_gwp<- ((sum(mango$End_SOC[19:36])/12)*44) + (((sum(mango$CH4[19:36])/12)*16)*28) + (((sum(mango$N2O_flux[19:36])/28)*44)*265)

mango_nicaragua_regen_gwp<- ((sum(mango$End_SOC[37:54])/12)*44) + (((sum(mango$CH4[37:54])/12)*16)*28) + (((sum(mango$N2O_flux[37:54])/28)*44)*265)

mango_nicaragua_organic_gwp<- ((sum(mango$End_SOC[55:72])/12)*44) + (((sum(mango$CH4[55:72])/12)*16)*28) + (((sum(mango$N2O_flux[55:72])/28)*44)*265)
...
grid.arrange(location_soc_plots, location_gwp_plots, ncol = 2, nrow = 1)


#### Try to make one data frame and one graph

location_df<- data.frame(Crop = c("Kernza", "Kernza", "Kernza", "Kernza", "Kernza", "Kernza", "Cotton", "Cotton","Cotton","Cotton","Cotton","Cotton","Cotton","Cotton", "Mango", "Mango", "Mango", "Mango", "Bison", "Bison", "Bison", "Bison"), 
Location = c("Scotland", "Scotland","Minnesota", "Minnesota", "Kansas", "Kansas", "China", "China","India", "India", "Texas", "Texas", "Peru", "Peru", "India", "India","Nicaragua", "Nicaragua", "South Dakota", "South Dakota","Brazil", "Brazil"),
                         Practice = c("Regenerative", "Organic","Regenerative", "Organic","Regenerative", "Organic", "Regenerative", "Organic","Regenerative", "Organic","Regenerative", "Organic", "Regenerative", "Organic", "Regenerative", "Organic","Regenerative", "Organic", "Regenerative", "Organic","Regenerative", "Organic"),
...
location_total_plot<- ggplot()+
  geom_bar(data = location_df, aes(y = GWP, x = Location, fill = Practice), stat = "identity", position = "dodge", width = 0.5)+
  geom_bar(data = location_df, aes(y = SOC, x = Location, fill = Practice), stat = "identity", position = "dodge", width = 0.5)+
  facet_wrap(~ Crop)+
  theme_classic()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
location_total_plot
...
cotton_manure_s<- sum(cotton$dSOC[140:123])
...
cotton_nomanure_s<-sum(cotton$dSOC[263:280])
bison_nocompost_s<-sum(cotton_tillage$dSOC[120:103])
...
bison_nocompost_g<-sum(cotton_tillage$GWP[120:103])
...
```{r}
cotton_mono_s<- sum(call_data$dSOC[361:378])
cotton_two_s<-sum(all_data$dSOC[379:396])
cotton_three_s<-sum(all_data$dSOC[397:414])
cotton_four_s<-sum(all_data$dSOC[415:432])


bison_mono_s<- sum(call_data$dSOC[60:43])
bison_two_s<-sum(all_data$dSOC[20:3])
bison_three_s<-sum(all_data$dSOC[40:23])
bison_four_s<-sum(all_data$dSOC[80:63])
...
cotton_regen_india_nt_s<- sum(cotton_tillage$dSOC[60:43])
cotton_regen_china_nt_s<-sum(cotton_tillage$dSOC[20:3])
cotton_regen_texas_nt_s<-sum(cotton_tillage$dSOC[40:23])
cotton_regen_peru_nt_s<-sum(cotton_tillage$dSOC[80:63])

cotton_till_india_s<-sum(cotton_tillage$dSOC[100:83])
cotton_till_china_s<-sum(cotton_tillage$dSOC[120:103])
cotton_till_texas_s<-sum(cotton_tillage$dSOC[160:143])
cotton_till_peru_s<-sum(cotton_tillage$dSOC[140:123])
